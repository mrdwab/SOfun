...
#' @param intervals A 2-row \code{matrix} with the same number of columns as the length of frequencies, with the first row being the lower class boundary, and the second row being the upper class boundary. Alternatively, \code{intervals} may be a column in your \code{data.frame}, and you may specify \code{sep} (and possibly, \code{trim}) to have the \code{GroupedMedian} function automatically create the required \code{matrix} for you.
#' @param sep Optional. If the \code{intervals} are represented by a character vector with a character separating the interval ranges.
#' @param trim Characters to trim from the vector before splitting. For example, if you are doing this on the output of \code{cut} (where, for some reason, you no longer have access to the original data), you can use the pre-set trim pattern \code{"cut"}.
