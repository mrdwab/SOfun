...
#' 
#' Moves all of the \code{NA} values in the rows or columns of a matrix to the end of the respective rows or columns.
#'  
#' @param inmat The input matrix.
#' @param by Should be either \code{"row"} or \code{"col"}, depending on if you want to shift non-\code{NA} values left or up.
#' @param outList Logical. Do you just want a \code{list} of the non-\code{NA} values? Defaults to \code{FALSE}.
#' @param fill While you're at it, do you want to replace \code{NA} with some other value?
#' @return Either a \code{matrix} with the same dimensions as the input matrix or a \code{list} with the same number of rows or columns as the input matrix (depending on the choice made in \code{by}).
#' @author Ananda Mahto
